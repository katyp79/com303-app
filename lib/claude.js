...
function systemPrompt(assignment, seed, redo) {
...
async function nextTurn({ assignment, student, history, maxTurns, seed, isRedo, attemptNumber, priorQuestions }) {
  const c = client();
  const messages = [];
  ...
  const resp = await c.messages.create({
    ...
    system: [{ type: "text", text: systemPrompt(assignment, seed, { isRedo, attemptNumber, priorQuestions }), cache_control: { type: "ephemeral" } }],
    messages
  });
  if (resp.usage) {
    const u = resp.usage;
